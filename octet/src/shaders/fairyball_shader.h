namespace octet
{
	namespace shaders
	{
		class fairyball_shader : public shader
		{
			...
			void init ()
			{
					...
					const char fragment_shader[] = SHADER_STR(
						...
						void main()
						{
							...
							rollingPotential = potential / numOfMetaballs;

							//if (potential > 1.0)
								//gl_FragColor = vec4 (blueColor, 1);
								gl_FragColor = vec4 ( mix (blueColor, whiteColor, rollingPotential * 16), 1);
							//else
								//discard;
						}
					);
